...
//		db.insert(DatabaseHelper.expTable, DatabaseHelper.expID, cv);
	...
	private static void FetchMiskatonicEncounters(SQLiteDatabase db,
		...
		db.execSQL(sqlInsertEnc + " (660,17,'You pass a room where a muscular, bored-looking man stands before a pile of groaning, fallen men. \"What about you?\" he calls out to you. \"Can you take a punch?\" If you allow him to drive his fist into your gut, lose 2 Stamina. If this does not knock you unconscious, take '''''' Sir William Brinton''s'''''' Ally card, or, if it''s not available, take $5 instead.')");
	}
	...
	private static void FetchFrenchHillEncounters(SQLiteDatabase db,
		...
		db.execSQL(sqlInsertEnc + " (315,23,'A woman poses by a window; the dim light revealing the outline of her beautiful figure, but hiding her face. \"Will you paint me?\" she asks. Make a </b>Luck (-2) check''''''. If you pass, you find a painting of the woman where she was standing. It has a Spell scribed on the back. Draw one Spell. If you fail, a monster appears!')");
		...
		db.execSQL(sqlInsertEnc + " (326,18,'A woman fleeing something runs right into you. As you try to help her up, she blindly attacks you. Pass a <b>Fight (-1) check</b> to restrain her; if you succeed, take ''''''Ruby Standish''s'''''' Ally card, or draw a Unique Item if it''s not available.')");
		...
		db.execSQL(sqlInsertEnc + " (379,11,'A gorgeous youth invites you into a meditation chamber. If you go in, you must lose a total of 4 Spells or Stamina, divided up as you choose, but you may search the Exhibit Item deck for the Triune Mask <i>(errata: should be Mask of the Three Fates)</i> card and take it. If you do not go in, the young witch casts a hex upon you, and you are <i>Barred from French Hill</i>.')");
	...
	private static void FetchEasttownEncounters(SQLiteDatabase db,
		...
		db.execSQL(sqlInsertEnc + " (205,8,'\"Nothing interesting''s happened here in a week,\" sighs a bored barfly as you step up to the bar. Taking stock of the mostly empty place, you believe it. As you think better and turn to go, the man takes a cue from the owner''s dirty look and stands to go with you, asking \"Where are you headed?\" Take ''''''Ryan Dean''s'''''' Ally card. If it''s not available, draw two Common Items instead.')");
		...
		db.execSQL(sqlInsertEnc + " (215,15,'If you succeed at a <b>Luck (-1) check</b>, then Deputy Dingby absentmindedly leaves you holding his gun. You may search the Common Item deck for a .45 Revolver <i>(errata: should read .38 Revolver)</i> card and take it.')");
	...
	private static void FetchDowntownEncounters(SQLiteDatabase db,
		...
		db.execSQL(sqlInsertEnc + " (6,10,'Make a <b>Will (-1) check</b>. If you succeed, you may take ''''''Anna Kaslow''s'''''' Ally card, or, if it''s not available, 2 Clue tokens.')");
